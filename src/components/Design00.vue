<template>
  <b-card bg-variant="light" :header="`Design: ${dropSlot}`" class="rounded-0" header-class="p-0 text-center">
    <drop @drop="drop">
  ...
  methods: {
    ...
    createSlotDropZone(node, slot = 'default') {
      if (slot === 'default') {
        ...
        const zone = 'inside';
        const content = this.createDropZone(this.dragContent, id, zone, true, slot);
    ...
    createDropZone(content, id, zone, asString = false, slot = "default") {
      const div = this.value.ownerDocument.createElement("div");
      ...
      div.firstElementChild.setAttribute(
        "v-bind:class",
        `{hover: owner.dropZone=='${zone}' && owner.dropSlot=='${slot}'}`
      );
      div.firstElementChild.setAttribute(
        "class",
